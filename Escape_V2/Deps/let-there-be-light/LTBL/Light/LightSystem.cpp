	...
	void LightSystem::RenderLights()
	{
		...
		for(unsigned int l = 0; l < numVisibleLights; l++)
		{
			...
			if(updateRequired)
			{
				...
				{
					m_lightAttenuationShader.bind(&m_lightAttenuationShader);
				...
				if(pLight->AlwaysUpdate())
				{
					...
					m_lightTempTexture.getTexture().bind(&m_lightTempTexture.getTexture());
				...
				else
				{
					...
					pLight->m_pStaticTexture->getTexture().bind(&pLight->m_pStaticTexture->getTexture());
